...
class FuncLogin extends Component {
...
constructor(props) {
  ...
  async login (){
    const {data} = await Auth.post("/login-funcionario",{login:this.state.login, senha:this.state.senha});
    this.setState({
  ...
  acessToken = (status, user) => {
    if(status){
      localStorage.setItem('func', user['login']);
              ...
              <CardGroup>
                  ...
                  <CardBody>
                    ...
                    <Form>
                      ...
                      <Row>
                        <Col xs="6">
                        {/* <Link to="/"> */}
                          <Button onClick={this.login.bind(this)} color="danger" className="px-4">Entrar</Button>
                        {/* </Link> */}
                ...
                <Card className="text-white bg-danger py-5 d-md-down-none" style={{ width: '44%' }}>
                  <CardBody className="text-center">
                    <div>
                      <h2>Seja um Funcionário</h2>
                      <p style={{paddingTop: "15px", paddingBottom: "40px"}} >Perfeito para seu dia a dia.</p>
                      <Link to="/admRegister">
                        <Button color="danger" className="mt-3" active tabIndex={-1}>Inscreva-se</Button>
                      </Link>
                    </div>
                  </CardBody>
                </Card>
